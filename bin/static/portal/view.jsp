...
<%@page
	import="es.elearningmedia.equella.data.WrappedUser,es.elearningmedia.equella.data.WrappedUser.TaskLink" %>
...
{
	String equellaUrl =  es.elearningmedia.equella.Configuration.instance().getEquellaUrl();
	String p = path;
	if (!path.startsWith(equellaUrl))
	{
		p = es.elearningmedia.equella.common.PathUtils.urlPath(es.elearningmedia.equella.Configuration.instance().getEquellaUrl(), path);
	}
	if (p.indexOf('?') == -1)
	{
		p += "?";
...
rel = es.elearningmedia.equella.common.BbUtil.getBlockRelativePath();
dir = rel + "portal/";
try
{
